  ...
  return (
    <div className={cn("flex flex-col gap-2 mt-4 rounded-lg", className)}>
      {messages.map(message => (
        // message box
        <div key={message.id} className={cn("flex flex-col gap-1 w-fit p-4 bg-zinc-100 dark:bg-zinc-900 rounded-md shadow-sm dark:shadow-none *:whitespace-pre-wrap", message.role === 'user' ? 'self-end' : 'self-start')}>
          {/* header: message role */}
